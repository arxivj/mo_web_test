...
class RotateCircle extends StatefulWidget {
  ...
  @override
  State<RotateCircle> createState() => _RotateCircleState();
}
...
class _RotateCircleState extends State<RotateCircle> {
  ...
  void initState() {
    ...
    rootBundle.load('rive/ai_right.riv').then(
          (data) async {
  ...
  Widget build(BuildContext context) {
    ...
    if (_riveArtboard != null) {
      processing!.value = AnimationCalculator.linearInterpolateToZeroOne(
          scrollPercentage: ssn.scrollPercentage, start: 2.21, end: 2.8) *
          100;
    }
    final perspectiveValue = 20 *
        pow(
            AnimationCalculator.linearInterpolateToMinusOneOne(
                scrollPercentage: ssn.scrollPercentage, start: 2.1, end: 3.2),
            3) -
        10;
